...
func _process(delta):
	current_state._animate(animation_tree, state_machine)
...
func _set_current(state:State, direction:Vector2):
	if current_state.can_finish:
		current_state._exit(animation_tree, state_machine)
		state._enter(animation_tree, state_machine, direction)
		current_state = state
	pass
